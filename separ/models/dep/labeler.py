...
class DependencyLabeler:
    ...
    def empirical(self, graph: CoNLL.Graph, known_labels: List[str], known_rels: List[str], REL: str) -> CoNLL.Graph:
        ...
        """
        labels, rels = self.encode(graph)
        labels = [label if label in known_labels else self.DEFAULT for label in labels]
