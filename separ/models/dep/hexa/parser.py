...
class HexaTaggingDependencyParser(DependencySLParser):
    ...
    class Labeler(DependencySLParser.Labeler):
        ...
        def encode(self, tree: CoNLL.Tree) -> Tuple[List[str], List[str], List[str], List[str]]:
            if self.proj:
                tree = tree.projectivize(self.proj)
            if len(tree) == 1:
                return ['>'], ['>'], [self.LEFT], tree.DEPREL
            ...
            tags, fences, cons = self.encode_bht(ctree, [], ['>'])
            return [self.BOS_TOKEN] + tags[:-1], [self.BOS_TOKEN] + fences, [self.BOS_TOKEN] + cons, tree.DEPREL
        
        def decode(self, tags: List[str], fences: List[str], cons: List[str], rels: List[str]) -> Tuple[List[Arc], bool]:
            if len(rels) == 1:
                return [Arc(0, 1, rels[0])], True 
            # Step 1: Obtain the BHT 
            tree, well_formed = self.decode_bht(tags[1:], fences[1:], cons[1:], len(rels))
    ...
    def transform(self, tree: CoNLL.Tree) -> CoNLL.Tree:
        if not tree.transformed:
            tree.HEXA, tree.FENCE, tree.CON, tree.REL = self.lab.encode(tree)
    ...
    def build(
        ...
        **_
        ...
        hexas, fences, cons, rels = map(flatten, zip(*bar(map(labeler.encode, data), total=len(data), leave=False, desc=f'{cls.NAME}[encode]')))
