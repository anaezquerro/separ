...
from separ.utils import flatten, Config, parallel, create_mask, DependencyMetric, acc, avg
...
from separ.models.dep.bracket.model import BracketDependencyModel
    ...
    class Labeler(DependencyLabeler):
        ...
        def test(self, graph: CoNLL.Graph) -> bool:
            if len(graph.planes) <= self.k:
                ...
                planar = graph.planarize(self.k)
                brackets, rels = self.encode(planar)
                rec1 = planar.rebuild(self.decode(brackets, rels))
                rec2, well_formed = self.decode_postprocess(brackets, rels)
