...
class PoSDependencyModel(Model):
    ...
    def __init__(
        ...
        if tag_conf is not None:
            self.tag = FFN(self.hidden_size, tag_conf.vocab_size) 
        else:
            self.tag = nn.Identity()
    ...
    def loss(
        ...
        """  
        loss = self.criteria(s_index, indexes) + self.criteria(s_rel, rels) 
        if self.tag_conf is not None:
            loss += self.criteria(s_tag, tags)
        return loss 
