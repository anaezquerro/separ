...
from separ.models.dep.pos.model import PoSDependencyModel
from separ.utils import Config, flatten, parallel, create_mask, avg, acc, DependencyMetric
from separ.data import CoNLL, AbstractTokenizer, Tokenizer, PretrainedTokenizer, CharacterTokenizer
from separ.structs import Arc, forms_cycles
...
class PoSDependencyParser(Parser):
    ...
    def _pred(
        ...
        """
        rec, well_formed = self.labeler.decode_postprocess(self.INDEX.decode(index_pred), self.REL.decode(rel_pred), self.TAG.decode(tag_pred))
    ...
    def train_step(
        ...
        return loss, dict(INDEX=acc(s_index, indexes), REL=acc(s_rel, rels), TAG=acc(s_tag, tags))
    ...
    def control_step(
        ...
        control = dict(INDEX=acc(index_preds, indexes), REL=acc(rel_preds, rels), TAG=acc(tag_preds, tags), loss=loss.item(), well_formed=avg(well_formed)*100)
    ...
    def build(
        ...
        **_
        ...
        model = cls.MODEL(enc_conf, *in_confs, index_tkz.conf, rel_tkz.conf, tag_tkz.conf).to(device)
        return cls(model, input_tkzs, [index_tkz, rel_tkz, tag_tkz], device)
