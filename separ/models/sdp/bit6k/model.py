...
class Bit6kSemanticModel(IndexSemanticModel):
    ...
    def __init__(
        ...
        super(IndexSemanticModel).__init__(enc_conf, word_conf, tag_conf, char_conf)
        self.bit_conf = bit_conf
        self.rel_conf = rel_conf
        self.confs = [enc_conf, word_conf, tag_conf, char_conf, bit_conf, rel_conf]
        
        self.bit = FFN(enc_conf.hidden_size, bit_conf.vocab_size)
        self.head = FFN(enc_conf.hidden_size, enc_conf.hidden_size//2)
        self.dep = FFN(enc_conf.hidden_size, enc_conf.hidden_size//2)
    ...
    def loss(self, s_bit: torch.Tensor, s_rel: torch.Tensor, bits: torch.Tensor, rels: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self.criteria(s_bit, bits.to(torch.long)) + self.criteria(s_rel, rels.to(torch.long))
