...
class BracketSemanticModel(IndexSemanticModel):
    def __init__(
        ...
        self.bracket = FFN(enc_conf.hidden_size, bracket_conf.vocab_size)
    ...
    # inherit: rel_score, rel_pred, rel_post
    ...
    def loss(self, s_bracket: torch.Tensor, s_rel: torch.Tensor, brackets: torch.Tensor, rels: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self.criteria(s_bracket, brackets.to(torch.long)) + self.criteria(s_rel, rels.to(torch.long))
