...
import os, torch, pickle, time, logging 
...
from trasepar.utils import to, bar, logger, init_folder, History, Epoch, Metric, merge, fdict, scale_dict, isna, filename, folderpath, SEED 
from trasepar.utils.metric import Metric
...
class Parser:
    ...
    def train(
        ...
        **_
        ...
        log.info(self)
        log.info(self.model)
        log.info(f'Saving training at {path}\n' + '\n'.join(f'{name} = {repr(value)}' for name, value in locals().items() if name not in ['self', '_', 'log']))
        self.optimizer = self.OPTIMIZER(self.model.parameters(), lr=lr)
        self.save(f'{path}/parser.pt')
        
        # prepare data 
        train = self.build_data(train, num_workers)
        dev, *test = map(lambda x: self.build_data(x, num_workers), [dev, *test])
        ...
        for i in range(self.epochs, epochs):
            ...
            if isna(loss):
                log.warning(f'Optimization issue: the loss is nan')
                break 
            ...
            # evaluation step 
            dev_control, dev_metric = self.control(dev, batch_size, num_workers=num_workers, log=log, path=path)
        ...
        self.load_state(f'{path}/parser.pt')
        self.trained = True 
        self.save(f'{path}/parser.pt')
    ...
    def evaluate(
        ...
        with bar(total=len(data), desc=f'eval:{data.name}', leave=False) as pbar:
            for inputs, _, masks, sents in loader:
                ...
                elapsed += (end-start)
        pred = data.__class__(preds, None)
    ...
    def predict(
        ...
        pred = data.__class__(preds, path or None)
        if path:
            pred.save(path)
        torch.cuda.empty_cache()
        return pred, elapsed 
    ...
    def reference(
        ...
        with bar(total=len(data), desc=f'ref:{data.name}', leave=False) as pbar:
            for _, targets, masks, sents in loader:
                ...
                pbar.update(len(sents))
                elapsed += (end-start)
        metric = self.METRIC(data.__class__(preds, data.name), data, num_workers=num_workers)
        log.info(f'{data.name}: {metric} [{data.n_tokens/elapsed:.2f} token/s, {len(data)/elapsed:.2f} sent/s, {elapsed:.2f}s elapsed]')
        return metric 
    ...
    def load(cls, path: str, device: str = 'cuda:0'):
        ...
        parser.optimizer.load_state_dict(parser.optimizer_state)
        delattr(parser, 'optimizer_state')
        parser.device = device 
        return parser 
    
    def load_state(self, path: str):
        states = torch.load(path, map_location=self.device)
        self.model.load_state_dict(states.state)
        if self.optimizer is not None:
            self.optimizer.load_state_dict(states.optimizer_state)
        
    @classmethod
    def add_arguments(cls, argparser: ArgumentParser):
        argparser.add_argument('-p', '--path', type=str, help='Parser path')
        argparser.add_argument('-c', '--conf', type=str, default=None, help='Configuration file')
        argparser.add_argument('--data', type=str, default=None, help='Building data')
        ...
        argparser.add_argument('--seed', type=int, default=SEED, help='Seed')
        return argparser
    
    def build_data(self, data: Union[str, AbstractDataset], num_workers: int = 1) -> AbstractDataset:
        if isinstance(data, str):
            data = self.load_data(data, num_workers=num_workers)
    ...
    @classmethod
    def load_data(cls, path: str, num_workers: int):
        data = None
        if isinstance(cls.DATASET, list):
            ext = path.split('.')[-1] # obtain extension 
            for b in cls.DATASET:
                if b.EXTENSION == ext:
                    data = b.from_file(path, num_workers)
        else:
            data = cls.DATASET.from_file(path, num_workers)
        if data is None:
            raise NotImplementedError(f'No method implemented for dataset {path}')
        return data 
