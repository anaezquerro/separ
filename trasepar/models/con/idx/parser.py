...
from trasepar.models.con.idx.model import IndexConstituencyModel
from trasepar.utils import ConstituencyMetric, create_mask, acc, avg, Config, flatten, parallel
from trasepar.data import PTB, AbstractTokenizer, Tokenizer, CharacterTokenizer, PretrainedTokenizer
...
class IndexConstituencyParser(Parser):
    ...
    def train_step(
        ...
        s_index, s_con, s_leaf = self.model(inputs[0], inputs[1:], mask)
        loss = self.model.loss(s_index, s_con, s_leaf, indexes, cons, leaves)
        return loss, dict(INDEX=acc(s_index, indexes), CON=acc(s_con, cons), LEAF=acc(s_leaf, leaves))
    ...
    def pred_step(
        ...
        index_preds, con_preds, leaf_preds = self.model.predict(inputs[0], inputs[1:], *masks)
        preds, _ = zip(*map(self._pred, trees, index_preds.split(lens), con_preds.split(lens), leaf_preds.split(lens)))
        return preds 
