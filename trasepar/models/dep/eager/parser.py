...
from trasepar.models.dep.eager.model import ArcEagerDependencyModel
from trasepar.utils import DependencyMetric, create_mask, acc, Config, parallel, flatten, split
from trasepar.data import CoNLL, Tokenizer, PretrainedTokenizer, CharacterTokenizer
from trasepar.structs import Arc, adjacent_from_arcs, candidates_no_cycles
...
class ArcEagerDependencyParser(Parser):
    ...
    class Oracle:
        ...
        def state(self) -> ArcEagerDependencyParser.State:
            stack = self.stack[-min(len(self.stack), self.stack):]
            if len(stack) < self.stack:
                stack = [0 for _ in range(self.stack - len(stack))] + stack
            buffer = self.buffer[:min(self.buffer, len(self.buffer))]
            if len(buffer) < self.buffer:
                buffer += [self.n+1 for _ in range(self.buffer - len(buffer))]
    ...
    def _pred(self, graph: CoNLL.Graph, oracle: Oracle) -> CoNLL.Graph:
        arcs = self.system.postprocess(list(oracle.recovered), len(graph))
        pred = graph.rebuild(arcs)
        if self.proj:
    ...
    def train_step(self, inputs: List[torch.Tensor], targets: List[torch.Tensor], _: List[torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        return loss, dict(ACTION=acc(s_action, actions), REL=acc(s_rel, rels))
    ...
    def pred_step(self, inputs: List[torch.Tensor], _: List[torch.Tensor], graphs: List[CoNLL.Graph]) -> List[CoNLL.Graph]:
        ...
        oracles = [self.Oracle(self.stack, self.buffer, self.proj).reset(len(graph)) for graph in graphs]
    ...
    def control_step(
        ...
        actions, rels, states = targets
        loss, embed, action_preds, rel_preds = self.model.control(inputs[0], inputs[1:], states)
        control = dict(loss=loss.item(), ACTION=acc(action_preds, actions), REL=acc(rel_preds, rels))
        oracles = [self.Oracle(self.stack, self.buffer).reset(len(graph)) for graph in graphs]
        while not all(oracle.final for oracle in oracles):
            states = [oracle.state.tensor.to(self.device).unsqueeze(0) for oracle in oracles]
            action_preds, rel_preds = self.model.predict(embed, states)
            action_preds = list(map(self.ACTION.decode, action_preds.unbind(0)))
            ...
            for oracle, action_pred, rel_pred in zip(oracles, action_preds, rel_preds):
                if not oracle.final:
                    oracle(action_pred, rel_pred)
        return control, map(self._pred, graphs, oracles)
    ...
    def build(
        ...
        model = cls.MODEL(enc_conf, *in_confs, action_tkz.conf, rel_conf, stack, buffer).to(device)
        model.confs = [enc_conf, *in_confs, action_tkz.conf, rel_conf, stack, buffer]
        return cls(model, input_tkzs, [action_tkz, rel_tkz], stack, buffer, proj, device)
