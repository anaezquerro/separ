from typing import Iterator, List, Callable, Union, Dict, Iterable
import torch, random, os, shutil, subprocess, tempfile
import numpy as np
...
def interleave(x1: list, x2: list) -> list:
    x1, x2 = x1.copy(), x2.copy()
    res = []
    while not (len(x1) == len(x2) == 0):
        if len(x1) > 0:
            res.append(x1.pop(0))
        if len(x2) > 0:
            res.append(x2.pop(0))
...
def create_2d_mask(lens: List[int], bos: bool = False, eos: bool = False) -> torch.Tensor:
...
def save_excel(dfs: Dict[str, pd.DataFrame], path: str):
    with pd.ExcelWriter(path) as writer:
        for name, df in dfs.items():
            df.to_excel(writer, sheet_name=name.replace('/', '|'))
            


def isna(x: float) -> bool:
    return torch.isnan(torch.tensor([x])).any().item()